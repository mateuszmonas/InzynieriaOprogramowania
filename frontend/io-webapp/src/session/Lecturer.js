...
const Lecturer = (props) => {
  ...
  const [approvalSocket, setApprovalSocket] = React.useState(
    Socket.connect(
      props.username,
      props.approvalRoomID,
      props.guestID,
      "judge",
      true,
      false,
    ...
    (async () => {
      await fetch("http://localhost:8080/session/participant-list", {
        ...
        .then((data) => {
          ...
          setParticipants([{id: "", username: data.leaderAccountName, approved: true}, ...data.participants]);
          setChat(false);
