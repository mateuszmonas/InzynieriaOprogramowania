...
const connect = (event) => {
    ...
    //passcode = document.querySelector('#passcode').value.trim()

    if (username) {
        const login = document.querySelector('#login')
        login.classList.add('hide')

        const meetingcode = document.querySelector('#meetingcode')
        meetingcode.classList.add('hide')
...
const onConnected = () => {
    ...
    stompClient.send("http://localhost:8080/app/chat.newUser",
        {},
...
const sendMessage = (event) => {
    ...
    if (messageContent && stompClient) {
        ...
        stompClient.send("http://localhost:8080/app/chat.send", {}, JSON.stringify(chatMessage))
        messageInput.value = ''
    }
    event.preventDefault();
}
...
const passcodeForm = document.querySelector('#meetingcode-form')
passcodeForm.addEventListener('submit', connect, true)
